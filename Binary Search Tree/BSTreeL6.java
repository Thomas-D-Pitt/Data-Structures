...
class BSTreeL6<T>
{
  ...
  private void calcLevelCounts( BSTNode root, int levelCounts[], int level )
  {
	...
	public BSTreeL6<T> makeBalancedCopyOf( )
	{
		...
		addNodesBalanced(balancedBST, keys, 0, keys.size()-1);
	...
	private void treeToArray(ArrayList<T> arr, BSTNode<T> node){
		if (node == null)
			return;
		
		treeToArray(arr, node.left);
		arr.add(node.key);
		treeToArray(arr, node.right);
	}

	private void addNodesBalanced(BSTreeL6<T> tree, ArrayList<T> arr, int low, int high){
		if (low > high)
			return;

		int pos = low + (high-low)/2;
		tree.add(arr.get(pos));

		addNodesBalanced(tree, arr, low, pos-1);
		addNodesBalanced(tree, arr, pos+1, high);
	}
	...
	{	
		...
		if (thisRoot == otherRoot && otherRoot == null)
			return true;

		if (thisRoot.key != otherRoot.key)
			return false;

		return equals(thisRoot.left, otherRoot.left) && equals(thisRoot.right, otherRoot.right);
	}
} // END BSTreeL6 CLASS